...
import { FAL_IMAGE_SIZE_ENUMS, parseImageSize } from "@/domains/generation/lib/image-utils"
import { createBrowserFalClient } from "@/domains/fal/lib/fal-client"
import { callOpenAIImageAPI } from "@/domains/openai/lib/openai-image-api"
import { callOpenAIResponsesAPI } from "@/domains/openai/lib/openai-responses-api"
...
import { getNewApiImagesUrl, getNewApiModelsUrl } from "@/domains/newapi/lib/newapi-endpoint"
...
async function callFalAPI(provider: ProviderConfig, params: GenerationParams): Promise<string[]> {
  ...
  try {
    const result = await falClient.subscribe(modelId as any, {
      ...
      onQueueUpdate: (update: { request_id?: string; status?: string; queue_position?: number }) => {
        void logDebug({
