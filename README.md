...
一个基于 Next.js 15 和 React 19 构建的多供应商 AI 图片生成与编辑界面。项目内置 FAL Queue、OpenAI Images 以及可自定义的 NewAPI 渠道，提供批量上传、参数化生成、结果预览、历史存档等端到端功能，适合快速搭建内部图片生产或创意工具。

## 功能特性
- 多供应商支持：在同一界面切换 FAL、OpenAI 与自建 NewAPI 服务，灵活配置端点和鉴权信息。
...
### FAL Queue
- `API Key`：FAL 平台生成的密钥。
- `API 端点`：默认 `https://queue.fal.run/fal-ai/flux/dev`，可替换为任意队列端点或模型 ID，系统会自动解析。
- `启用`：开启后即加入可选供应商列表。
...
- `POST /api/fal/proxy`：转发请求到 FAL 队列，支持轮询任务状态与结果。
...
- **同步模式下未返回图片**：FAL 队列可能需要异步拉取，尝试开启同步模式或等待轮询完成。
